/* eslint-disable react/no-unescaped-entities */
// import { useState } from 'react'
// import reactLogo from './assets/react.svg'
// import viteLogo from '/vite.svg'
// import './App.css'
import './main.css'

function App() {
  // const [count, setCount] = useState(0)

  return (
    <>
      <div className="style-0">
        <div className="style-1">
          <div className="style-2">
            <nav className="style-3">
              <a className="style-4" href="/">
                <img
                  alt="Mantle logo"
                  width={138}
                  height={40}
                  decoding="async"
                  data-nimg={1}
                  className="style-5"
                  src="/logo-lockup.svg"
                />
              </a>
              <div className="style-6">
                <div
                  role="menubar"
                  className="style-7"
                  tabIndex={0}
                  data-orientation="horizontal"
                >
                  <button
                    type="button"
                    role="menuitem"
                    aria-haspopup="menu"
                    aria-expanded="false"
                    data-state="closed"
                    className="style-8"
                    tabIndex={-1}
                    data-orientation="horizontal"
                    data-radix-collection-item=""
                  >
                    <span className="style-9">Ecosystem</span>
                    <svg
                      viewBox="0 0 12 13"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                      className="style-10"
                    >
                      <path
                        d="m3 5 3 3 3-3"
                        stroke="white"
                        strokeWidth="1.2"
                        className="style-11"
                      />
                    </svg>
                  </button>
                  <button
                    type="button"
                    role="menuitem"
                    aria-haspopup="menu"
                    aria-expanded="false"
                    data-state="closed"
                    className="style-12"
                    tabIndex={-1}
                    data-orientation="horizontal"
                    data-radix-collection-item=""
                  >
                    <span className="style-13">Bridge</span>
                    <svg
                      viewBox="0 0 12 13"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                      className="style-14"
                    >
                      <path
                        d="m3 5 3 3 3-3"
                        stroke="white"
                        strokeWidth="1.2"
                        className="style-15"
                      />
                    </svg>
                  </button>
                  <button
                    type="button"
                    role="menuitem"
                    aria-haspopup="menu"
                    aria-expanded="false"
                    data-state="closed"
                    className="style-16"
                    tabIndex={-1}
                    data-orientation="horizontal"
                    data-radix-collection-item=""
                  >
                    <span className="style-17">Developers</span>
                    <svg
                      viewBox="0 0 12 13"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                      className="style-18"
                    >
                      <path
                        d="m3 5 3 3 3-3"
                        stroke="white"
                        strokeWidth="1.2"
                        className="style-19"
                      />
                    </svg>
                  </button>
                  <button
                    type="button"
                    role="menuitem"
                    aria-haspopup="menu"
                    aria-expanded="false"
                    data-state="closed"
                    className="style-20"
                    tabIndex={-1}
                    data-orientation="horizontal"
                    data-radix-collection-item=""
                  >
                    <span className="style-21">mETH</span>
                    <svg
                      viewBox="0 0 12 13"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                      className="style-22"
                    >
                      <path
                        d="m3 5 3 3 3-3"
                        stroke="white"
                        strokeWidth="1.2"
                        className="style-23"
                      />
                    </svg>
                  </button>
                  <button
                    type="button"
                    role="menuitem"
                    aria-haspopup="menu"
                    aria-expanded="false"
                    data-state="closed"
                    className="style-24"
                    tabIndex={-1}
                    data-orientation="horizontal"
                    data-radix-collection-item=""
                  >
                    <span className="style-25">Governance</span>
                    <svg
                      viewBox="0 0 12 13"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                      className="style-26"
                    >
                      <path
                        d="m3 5 3 3 3-3"
                        stroke="white"
                        strokeWidth="1.2"
                        className="style-27"
                      />
                    </svg>
                  </button>
                  <button
                    type="button"
                    role="menuitem"
                    aria-haspopup="menu"
                    aria-expanded="false"
                    data-state="closed"
                    className="style-28"
                    tabIndex={-1}
                    data-orientation="horizontal"
                    data-radix-collection-item=""
                  >
                    <span className="style-29">Journey</span>
                    <svg
                      viewBox="0 0 12 13"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                      className="style-30"
                    >
                      <path
                        d="m3 5 3 3 3-3"
                        stroke="white"
                        strokeWidth="1.2"
                        className="style-31"
                      />
                    </svg>
                  </button>
                  <button
                    type="button"
                    role="menuitem"
                    aria-haspopup="menu"
                    aria-expanded="false"
                    data-state="closed"
                    className="style-32"
                    tabIndex={-1}
                    data-orientation="horizontal"
                    data-radix-collection-item=""
                  >
                    <span className="style-33">Resources</span>
                    <svg
                      viewBox="0 0 12 13"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                      className="style-34"
                    >
                      <path
                        d="m3 5 3 3 3-3"
                        stroke="white"
                        strokeWidth="1.2"
                        className="style-35"
                      />
                    </svg>
                  </button>
                </div>
              </div>
              <div className="style-36">
                <div className="style-37">
                  <a
                    target="_blank"
                    className="style-38"
                    href="https://twitter.com/0xMantle"
                  >
                    <img
                      alt="nav-twitter"
                      loading="lazy"
                      width={16}
                      height={14}
                      decoding="async"
                      data-nimg={1}
                      src="/X.10d54851.svg"
                      className="style-39"
                    />
                  </a>
                  <a
                    target="_blank"
                    className="style-40"
                    href="https://discord.com/invite/0xMantle"
                  >
                    <img
                      alt="nav-discord"
                      loading="lazy"
                      width={16}
                      height={13}
                      decoding="async"
                      data-nimg={1}
                      src="/dicord.121e4489.svg"
                      className="style-41"
                    />
                  </a>
                  <a
                    target="_blank"
                    className="style-42"
                    href="https://t.me/mantlenetwork"
                  >
                    <img
                      alt="nav-telegram"
                      loading="lazy"
                      width={16}
                      height={16}
                      decoding="async"
                      data-nimg={1}
                      src="/telegram.19a5a5db.svg"
                      className="style-43"
                    />
                  </a>
                </div>
                <div className="style-44">
                  <div
                    role="menubar"
                    className="style-45"
                    tabIndex={0}
                    data-orientation="horizontal"
                  >
                    <button
                      type="button"
                      role="menuitem"
                      aria-haspopup="menu"
                      aria-expanded="false"
                      data-state="closed"
                      className="style-46"
                      tabIndex={-1}
                      data-orientation="horizontal"
                      data-radix-collection-item=""
                    >
                      <span className="style-47">en</span>
                      <svg
                        viewBox="0 0 12 13"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                        className="style-48"
                      >
                        <path
                          d="m3 5 3 3 3-3"
                          stroke="white"
                          strokeWidth="1.2"
                          className="style-49"
                        />
                      </svg>
                    </button>
                  </div>
                </div>
              </div>
            </nav>
            <nav className="style-50">
              <div className="style-51" />
              <div className="style-52">
                <div className="style-53">
                  <a href="/" className="style-54">
                    <img
                      alt="Mantle logo"
                      width={36}
                      height={36}
                      decoding="async"
                      data-nimg={1}
                      className="style-55"
                      src="/logo-light.svg"
                    />
                  </a>
                  <div className="style-56">
                    <div
                      role="menubar"
                      className="style-57"
                      tabIndex={0}
                      data-orientation="horizontal"
                    >
                      <button
                        type="button"
                        role="menuitem"
                        aria-haspopup="menu"
                        aria-expanded="false"
                        data-state="closed"
                        className="style-58"
                        tabIndex={-1}
                        data-orientation="horizontal"
                        data-radix-collection-item=""
                      >
                        <span className="style-59">en</span>
                        <svg
                          viewBox="0 0 12 13"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                          className="style-60"
                        >
                          <path
                            d="m3 5 3 3 3-3"
                            stroke="white"
                            strokeWidth="1.2"
                            className="style-61"
                          />
                        </svg>
                      </button>
                    </div>
                    <button aria-label="Mantle Menu Button" className="style-62">
                      <svg
                        width={30}
                        height={24}
                        viewBox="0 0 30 24"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                        className="style-63"
                      >
                        <rect
                          x="0.213745"
                          y="9.98193"
                          width="29.6622"
                          height="2.5"
                          fill="#D9D9D9"
                          className="style-64"
                        />
                        <rect
                          x="0.213745"
                          y="0.03125"
                          width="29.6622"
                          height="2.5"
                          fill="#D9D9D9"
                          className="style-65"
                        />
                        <rect
                          x="0.213745"
                          y="19.9329"
                          width="29.6622"
                          height="2.5"
                          fill="#D9D9D9"
                          className="style-66"
                        />
                      </svg>
                    </button>
                  </div>
                </div>
              </div>
            </nav>
            <div className="style-67">
              <h1 className="style-68">Mass adoption of token-governed technologies</h1>
              <p className="style-69">
                With Mantle Network, an Ethereum rollup, Mantle Treasury and a token
                holder governed roadmap for products and initiatives.
              </p>
              <div className="style-70">
                <a
                  href="/bridge"
                  target="_blank"
                  rel="noreferrer noopener"
                  className="style-71"
                >
                  <button type="button" className="style-72">
                    Bridge to Mantle
                  </button>
                </a>
                <a
                  href="https://www.mantle.xyz/ecosystem"
                  target="_blank"
                  rel="noreferrer noopener"
                  className="style-73"
                >
                  <button type="button" className="style-74">
                    Explore
                  </button>
                </a>
                <a
                  href="https://docs-v2.mantle.xyz/"
                  target="_blank"
                  rel="noreferrer noopener"
                  className="style-75"
                >
                  <button type="button" className="style-76">
                    View Docs
                  </button>
                </a>
              </div>
              <div className="style-77">
                For more information
                <a
                  href="https://docs.mantle.xyz/network/introduction/overview"
                  target="_blank"
                  rel="noreferrer noopener"
                  className="style-78"
                >
                  View Docs
                </a>
              </div>
            </div>
            <div className="style-79">
              <div className="style-80">
                <div className="style-81">
                  <div className="style-82">
                    <a
                      className="style-83"
                      href="/blog/announcements/mantle-sharding-with-ethena"
                    >
                      <img
                        alt="Background style Mantle image 3"
                        loading="lazy"
                        decoding="async"
                        data-nimg="fill"
                        className="style-84"
                        sizes="100vw"
                        // srcSet="/_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F72e80299-c67b-4de7-af7d-bde6589f8452.jpeg&w=640&q=75 640w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F72e80299-c67b-4de7-af7d-bde6589f8452.jpeg&w=750&q=75 750w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F72e80299-c67b-4de7-af7d-bde6589f8452.jpeg&w=828&q=75 828w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F72e80299-c67b-4de7-af7d-bde6589f8452.jpeg&w=1080&q=75 1080w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F72e80299-c67b-4de7-af7d-bde6589f8452.jpeg&w=1200&q=75 1200w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F72e80299-c67b-4de7-af7d-bde6589f8452.jpeg&w=1920&q=75 1920w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F72e80299-c67b-4de7-af7d-bde6589f8452.jpeg&w=2048&q=75 2048w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F72e80299-c67b-4de7-af7d-bde6589f8452.jpeg&w=3840&q=75 3840w"
                        src="/72e80299-c67b-4de7-af7d-bde6589f8452.jpg"
                      />
                    </a>
                    <a
                      target="_blank"
                      rel="noreferrer noopener"
                      className="style-85"
                      href="https://www.mantle.xyz/rewards-station"
                    >
                      <button type="button" className="style-86">
                        Participate Now
                      </button>
                    </a>
                  </div>
                </div>
                <div className="style-87">
                  <div className="style-88">
                    <button className="style-89" />
                  </div>
                </div>
              </div>
              <div className="style-90">
                <div className="style-91">
                  <div className="style-92">
                    <a
                      className="style-93"
                      href="/blog/announcements/mantle-sharding-with-ethena"
                    >
                      <img
                        alt="Background style Mantle image 3"
                        loading="lazy"
                        decoding="async"
                        data-nimg="fill"
                        className="style-94"
                        sizes="100vw"
                        // srcSet="/_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F72e80299-c67b-4de7-af7d-bde6589f8452.jpeg&w=640&q=75 640w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F72e80299-c67b-4de7-af7d-bde6589f8452.jpeg&w=750&q=75 750w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F72e80299-c67b-4de7-af7d-bde6589f8452.jpeg&w=828&q=75 828w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F72e80299-c67b-4de7-af7d-bde6589f8452.jpeg&w=1080&q=75 1080w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F72e80299-c67b-4de7-af7d-bde6589f8452.jpeg&w=1200&q=75 1200w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F72e80299-c67b-4de7-af7d-bde6589f8452.jpeg&w=1920&q=75 1920w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F72e80299-c67b-4de7-af7d-bde6589f8452.jpeg&w=2048&q=75 2048w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F72e80299-c67b-4de7-af7d-bde6589f8452.jpeg&w=3840&q=75 3840w"
                        src="/72e80299-c67b-4de7-af7d-bde6589f8452.jpeg"
                      />
                    </a>
                    <a
                      target="_blank"
                      rel="noreferrer noopener"
                      className="style-95"
                      href="https://www.mantle.xyz/rewards-station"
                    >
                      <button type="button" className="style-96">
                        Participate Now
                      </button>
                    </a>
                  </div>
                  <div className="style-97">
                    <a
                      className="style-98"
                      href="/blog/announcements/mantle-completes-mainnet-v2-tectonic-upgrade"
                    >
                      <img
                        alt="Background style Mantle image 3"
                        loading="lazy"
                        decoding="async"
                        data-nimg="fill"
                        className="style-99"
                        sizes="100vw"
                        // srcSet="/_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fbe93f672-779f-4d1c-b74a-b005e36fa4ce.webp&w=640&q=75 640w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fbe93f672-779f-4d1c-b74a-b005e36fa4ce.webp&w=750&q=75 750w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fbe93f672-779f-4d1c-b74a-b005e36fa4ce.webp&w=828&q=75 828w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fbe93f672-779f-4d1c-b74a-b005e36fa4ce.webp&w=1080&q=75 1080w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fbe93f672-779f-4d1c-b74a-b005e36fa4ce.webp&w=1200&q=75 1200w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fbe93f672-779f-4d1c-b74a-b005e36fa4ce.webp&w=1920&q=75 1920w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fbe93f672-779f-4d1c-b74a-b005e36fa4ce.webp&w=2048&q=75 2048w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fbe93f672-779f-4d1c-b74a-b005e36fa4ce.webp&w=3840&q=75 3840w"
                        src="/be93f672-779f-4d1c-b74a-b005e36fa4ce.webp"
                      />
                    </a>
                    <a
                      target="_blank"
                      rel="noreferrer noopener"
                      className="style-100"
                      href="/bridge"
                    >
                      <button type="button" className="style-101">
                        Bridge to Mantle
                      </button>
                    </a>
                  </div>
                  <div className="style-102">
                    <a className="style-103" href="javascript:void(0)">
                      <img
                        alt="Background style Mantle image 3"
                        loading="lazy"
                        decoding="async"
                        data-nimg="fill"
                        className="style-104"
                        sizes="100vw"
                        // srcSet="/_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F29438e05-9d10-41ac-a2e1-50c801c6705f.jpg&w=640&q=75 640w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F29438e05-9d10-41ac-a2e1-50c801c6705f.jpg&w=750&q=75 750w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F29438e05-9d10-41ac-a2e1-50c801c6705f.jpg&w=828&q=75 828w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F29438e05-9d10-41ac-a2e1-50c801c6705f.jpg&w=1080&q=75 1080w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F29438e05-9d10-41ac-a2e1-50c801c6705f.jpg&w=1200&q=75 1200w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F29438e05-9d10-41ac-a2e1-50c801c6705f.jpg&w=1920&q=75 1920w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F29438e05-9d10-41ac-a2e1-50c801c6705f.jpg&w=2048&q=75 2048w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F29438e05-9d10-41ac-a2e1-50c801c6705f.jpg&w=3840&q=75 3840w"
                        src="/29438e05-9d10-41ac-a2e1-50c801c6705f.jpg"
                      />
                    </a>
                    <a
                      target="_blank"
                      rel="noreferrer noopener"
                      className="style-105"
                      href="https://snapshot.org/#/bitdao.eth"
                    >
                      <button type="button" className="style-106">
                        More Details
                      </button>
                    </a>
                  </div>
                  <div className="style-107">
                    <a className="style-108" href="javascript:void(0)">
                      <img
                        alt="Background style Mantle image 3"
                        loading="lazy"
                        decoding="async"
                        data-nimg="fill"
                        className="style-109"
                        sizes="100vw"
                        // srcSet="/_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fcf56c23c-01ee-4fc3-9345-afd712afff0d.jpg&w=640&q=75 640w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fcf56c23c-01ee-4fc3-9345-afd712afff0d.jpg&w=750&q=75 750w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fcf56c23c-01ee-4fc3-9345-afd712afff0d.jpg&w=828&q=75 828w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fcf56c23c-01ee-4fc3-9345-afd712afff0d.jpg&w=1080&q=75 1080w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fcf56c23c-01ee-4fc3-9345-afd712afff0d.jpg&w=1200&q=75 1200w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fcf56c23c-01ee-4fc3-9345-afd712afff0d.jpg&w=1920&q=75 1920w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fcf56c23c-01ee-4fc3-9345-afd712afff0d.jpg&w=2048&q=75 2048w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fcf56c23c-01ee-4fc3-9345-afd712afff0d.jpg&w=3840&q=75 3840w"
                        src="/cf56c23c-01ee-4fc3-9345-afd712afff0d.jpg"
                      />
                    </a>
                    <a
                      target="_blank"
                      rel="noreferrer noopener"
                      className="style-110"
                      href="https://journey.mantle.xyz/en/nft/armory"
                    >
                      <button type="button" className="style-111">
                        Upgrade Now
                      </button>
                    </a>
                  </div>
                </div>
                <div className="style-112">
                  <div className="style-113">
                    <button className="style-114" />
                    <button className="style-115" />
                    <button className="style-116" />
                    <button className="style-117" />
                  </div>
                </div>
              </div>
              <div className="style-118">
                <div className="style-119">
                  <a
                    className="style-120"
                    href="/blog/announcements/mantle-completes-mainnet-v2-tectonic-upgrade"
                  >
                    <img
                      alt="Background style Mantle image 3"
                      loading="lazy"
                      decoding="async"
                      data-nimg="fill"
                      className="style-121"
                      sizes="100vw"
                      // srcSet="/_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fbe93f672-779f-4d1c-b74a-b005e36fa4ce.webp&w=640&q=75 640w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fbe93f672-779f-4d1c-b74a-b005e36fa4ce.webp&w=750&q=75 750w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fbe93f672-779f-4d1c-b74a-b005e36fa4ce.webp&w=828&q=75 828w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fbe93f672-779f-4d1c-b74a-b005e36fa4ce.webp&w=1080&q=75 1080w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fbe93f672-779f-4d1c-b74a-b005e36fa4ce.webp&w=1200&q=75 1200w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fbe93f672-779f-4d1c-b74a-b005e36fa4ce.webp&w=1920&q=75 1920w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fbe93f672-779f-4d1c-b74a-b005e36fa4ce.webp&w=2048&q=75 2048w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fbe93f672-779f-4d1c-b74a-b005e36fa4ce.webp&w=3840&q=75 3840w"
                      src="/right-1.webp"
                    />
                  </a>
                  <a
                    target="_blank"
                    rel="noreferrer noopener"
                    className="style-122"
                    href="/bridge"
                  >
                    <button type="button" className="style-123">
                      Bridge to Mantle
                    </button>
                  </a>
                </div>
                <div className="style-124">
                  <a className="style-125" href="javascript:void(0)">
                    <img
                      alt="Background style Mantle image 3"
                      loading="lazy"
                      decoding="async"
                      data-nimg="fill"
                      className="style-126"
                      sizes="100vw"
                      // srcSet="/_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F29438e05-9d10-41ac-a2e1-50c801c6705f.jpg&w=640&q=75 640w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F29438e05-9d10-41ac-a2e1-50c801c6705f.jpg&w=750&q=75 750w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F29438e05-9d10-41ac-a2e1-50c801c6705f.jpg&w=828&q=75 828w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F29438e05-9d10-41ac-a2e1-50c801c6705f.jpg&w=1080&q=75 1080w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F29438e05-9d10-41ac-a2e1-50c801c6705f.jpg&w=1200&q=75 1200w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F29438e05-9d10-41ac-a2e1-50c801c6705f.jpg&w=1920&q=75 1920w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F29438e05-9d10-41ac-a2e1-50c801c6705f.jpg&w=2048&q=75 2048w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2F29438e05-9d10-41ac-a2e1-50c801c6705f.jpg&w=3840&q=75 3840w"
                      src="/right-2.webp"
                    />
                  </a>
                  <a
                    target="_blank"
                    rel="noreferrer noopener"
                    className="style-127"
                    href="https://snapshot.org/#/bitdao.eth"
                  >
                    <button type="button" className="style-128">
                      More Details
                    </button>
                  </a>
                </div>
                <div className="style-129">
                  <a className="style-130" href="javascript:void(0)">
                    <img
                      alt="Background style Mantle image 3"
                      loading="lazy"
                      decoding="async"
                      data-nimg="fill"
                      className="style-131"
                      sizes="100vw"
                      // srcSet="/_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fcf56c23c-01ee-4fc3-9345-afd712afff0d.jpg&w=640&q=75 640w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fcf56c23c-01ee-4fc3-9345-afd712afff0d.jpg&w=750&q=75 750w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fcf56c23c-01ee-4fc3-9345-afd712afff0d.jpg&w=828&q=75 828w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fcf56c23c-01ee-4fc3-9345-afd712afff0d.jpg&w=1080&q=75 1080w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fcf56c23c-01ee-4fc3-9345-afd712afff0d.jpg&w=1200&q=75 1200w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fcf56c23c-01ee-4fc3-9345-afd712afff0d.jpg&w=1920&q=75 1920w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fcf56c23c-01ee-4fc3-9345-afd712afff0d.jpg&w=2048&q=75 2048w, /_next/image?url=https%3A%2F%2Fcms.mantle.xyz%2Fassets%2Fcf56c23c-01ee-4fc3-9345-afd712afff0d.jpg&w=3840&q=75 3840w"
                      src="/right-3.webp"
                    />
                  </a>
                  <a
                    target="_blank"
                    rel="noreferrer noopener"
                    className="style-132"
                    href="https://journey.mantle.xyz/en/nft/armory"
                  >
                    <button type="button" className="style-133">
                      Upgrade Now
                    </button>
                  </a>
                </div>
              </div>
            </div>
            <div className="style-134">
              <a href="/blog" className="style-135">
                <button type="button" className="style-136">
                  View all Updates
                </button>
              </a>
            </div>
            <div className="style-137">
              <h2 className="style-138">We build $MNT-powered products on Ethereum</h2>
              <div className="style-139">
                <div className="style-140">
                  <div width={530} height={500} className="style-141">
                    <img
                      alt="Mantle Network"
                      loading="eager"
                      decoding="async"
                      data-nimg="fill"
                      className="style-142"
                      sizes="100vw"
                      // srcSet="/_next/image?url=%2Fsecondary-3d-element-rotated.png&w=640&q=75 640w, /_next/image?url=%2Fsecondary-3d-element-rotated.png&w=750&q=75 750w, /_next/image?url=%2Fsecondary-3d-element-rotated.png&w=828&q=75 828w, /_next/image?url=%2Fsecondary-3d-element-rotated.png&w=1080&q=75 1080w, /_next/image?url=%2Fsecondary-3d-element-rotated.png&w=1200&q=75 1200w, /_next/image?url=%2Fsecondary-3d-element-rotated.png&w=1920&q=75 1920w, /_next/image?url=%2Fsecondary-3d-element-rotated.png&w=2048&q=75 2048w, /_next/image?url=%2Fsecondary-3d-element-rotated.png&w=3840&q=75 3840w"
                      src="/secondary-3d-element-rotated.png"
                    />
                  </div>
                  <div className="style-143" />
                  <div className="style-144">
                    <div className="style-145" />
                    <div className="style-146">
                      <h3 className="style-147">
                        Mantle Network
                        {/* */}
                        <span className="style-148">Mainnet</span>
                      </h3>
                      <div className="style-149">
                        <p className="style-150">
                          Build dApps with exceptional UX, all while relying on
                          Ethereum's unrivaled security, with our high-performance
                          Ethereum layer-2 network built with modular architecture.
                        </p>
                      </div>
                    </div>
                    <div className="style-151" />
                    <div className="style-152">
                      <div className="style-153">
                        <a
                          href="/bridge"
                          target="_blank"
                          rel="noreferrer noopener"
                          className="style-154"
                        >
                          <button type="button" className="style-155">
                            Bridge to Mantle
                          </button>
                        </a>
                        <a
                          href="http://docs.mantle.xyz"
                          target="_blank"
                          rel="noreferrer noopener"
                          className="style-156"
                        >
                          <button type="button" className="style-157">
                            Build On Mantle
                          </button>
                        </a>
                      </div>
                      <div className="style-158">
                        <div className="style-159" />
                        <div className="style-160">
                          <div className="style-161">
                            <div className="style-162" />
                            <div className="style-163">
                              <h3 className="style-164">
                                Hyperscale Performance
                                {/* */}
                              </h3>
                              <p className="style-165">
                                Low fees and faster finality — achieved by combining
                                roll-up technology with a decentralized data
                                availability layer (Mantle DA powered by EigenDA
                                technology).
                              </p>
                              <div className="style-166">
                                <a
                                  href="https://docs.mantle.xyz/network/introduction/concepts/data-availability"
                                  target="_blank"
                                  rel="noreferrer noopener"
                                  className="style-167"
                                >
                                  <button type="button" className="style-168">
                                    Learn More
                                  </button>
                                </a>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div className="style-169">
                          <div className="style-170">
                            <div className="style-171" />
                            <div className="style-172">
                              <h3 className="style-173">
                                Secured by Ethereum
                                {/* */}
                              </h3>
                              <p className="style-174">
                                Experience high throughput with unrivaled security with
                                Ethereum's roll-up technology — all in a familiar EVM
                                environment.
                              </p>
                              <div className="style-175">
                                <a
                                  href="https://www.mantle.xyz/blog/announcements/mantle-a-new-approach-to-scaling-ethereum"
                                  target="_blank"
                                  rel="noreferrer noopener"
                                  className="style-176"
                                >
                                  <button type="button" className="style-177">
                                    Learn More
                                  </button>
                                </a>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="style-178">
                  <div width={400} height={500} className="style-179">
                    <img
                      alt="Mantle LSP"

                      loading="eager"
                      decoding="async"
                      data-nimg="fill"
                      className="style-180"
                      sizes="100vw"
                      // srcSet="/_next/image?url=%2Fball.png&w=640&q=75 640w, /_next/image?url=%2Fball.png&w=750&q=75 750w, /_next/image?url=%2Fball.png&w=828&q=75 828w, /_next/image?url=%2Fball.png&w=1080&q=75 1080w, /_next/image?url=%2Fball.png&w=1200&q=75 1200w, /_next/image?url=%2Fball.png&w=1920&q=75 1920w, /_next/image?url=%2Fball.png&w=2048&q=75 2048w, /_next/image?url=%2Fball.png&w=3840&q=75 3840w"
                      src="/ball.png"
                    />
                  </div>
                  <div className="style-181">
                    <div className="style-182" />
                    <div className="style-183">
                      <div className="style-184">
                        <img
                          alt=""
                          loading="lazy"
                          width={44}
                          height={52}
                          decoding="async"
                          data-nimg={1}
                          src="/red-token.svg"
                          className="style-185"
                        />
                      </div>
                      <h3 className="style-186">
                        Mantle LSP
                        {/* */}
                      </h3>
                      <p className="style-187">
                        Mantle Liquidity Staking Protocol (LSP) is the second core
                        product of Mantle Ecosystem. Stake ETH and receive yield bearing
                        mETH (Mantle Staked Ether). Explore additional mETH yield
                        options on Mantle Network applications.
                      </p>
                      <a
                        href="https://mantle.xyz/meth"
                        target="_blank"
                        rel="noreferrer noopener"
                        className="style-188"
                      >
                        <button type="button" className="style-189">
                          Learn More
                        </button>
                      </a>
                    </div>
                  </div>
                </div>
                <div className="style-190">
                  <div width={500} height={380} className="style-191">
                    <img
                      alt="Next Mantle Product"

                      loading="eager"
                      decoding="async"
                      data-nimg="fill"
                      className="style-192"
                      sizes="100vw"
                      // srcSet="/_next/image?url=%2Fproduct-illustration.png&w=640&q=75 640w, /_next/image?url=%2Fproduct-illustration.png&w=750&q=75 750w, /_next/image?url=%2Fproduct-illustration.png&w=828&q=75 828w, /_next/image?url=%2Fproduct-illustration.png&w=1080&q=75 1080w, /_next/image?url=%2Fproduct-illustration.png&w=1200&q=75 1200w, /_next/image?url=%2Fproduct-illustration.png&w=1920&q=75 1920w, /_next/image?url=%2Fproduct-illustration.png&w=2048&q=75 2048w, /_next/image?url=%2Fproduct-illustration.png&w=3840&q=75 3840w"
                      src="/product-illustration.png"
                    />
                  </div>
                  <div className="style-193">
                    <div className="style-194" />
                    <div className="style-195">
                      <h3 className="style-196">
                        Next Mantle Product
                        {/* */}
                      </h3>
                      <p className="style-197">
                        Visit our forum to propose the next Mantle product. Synergize
                        with existing Mantle products, treasury, and ecosystem.
                      </p>
                      <a
                        href="https://forum.mantle.xyz/"
                        target="_blank"
                        rel="noreferrer noopener"
                        className="style-198"
                      >
                        <button type="button" className="style-199">
                          Visit our forum
                        </button>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="style-200">
              <h2 className="style-201">
                Mantle Governance: Token holders determine the strategic direction of
                the Mantle ecosystem
              </h2>
              <div className="style-202">
                <div className="style-203">
                  <div width={690} height={650} className="style-204">
                    <img
                      alt="Governance"

                      loading="eager"
                      decoding="async"
                      data-nimg="fill"
                      className="style-205"
                      sizes="100vw"
                      // srcSet="/_next/image?url=%2Felements_illustration.png&w=640&q=75 640w, /_next/image?url=%2Felements_illustration.png&w=750&q=75 750w, /_next/image?url=%2Felements_illustration.png&w=828&q=75 828w, /_next/image?url=%2Felements_illustration.png&w=1080&q=75 1080w, /_next/image?url=%2Felements_illustration.png&w=1200&q=75 1200w, /_next/image?url=%2Felements_illustration.png&w=1920&q=75 1920w, /_next/image?url=%2Felements_illustration.png&w=2048&q=75 2048w, /_next/image?url=%2Felements_illustration.png&w=3840&q=75 3840w"
                      src="/elements_illustration.png"
                    />
                  </div>
                  <div className="style-206">
                    <div className="style-207" />
                    <div className="style-208">
                      <h3 className="style-209">Governance</h3>
                      <p className="style-210">
                        All decisions, from launching new initiatives, to transferring
                        and recalling treasury resources, assigning and modifying powers
                        to operating teams, and implementing corrective actions, are
                        subject to proposals and voting by $MNT token holders.
                      </p>
                      <div className="style-211">
                        <a
                          href="https://forum.mantle.xyz/"
                          target="_blank"
                          rel="noreferrer noopener"
                          className="style-212"
                        >
                          <button type="button" className="style-213">
                            Forum
                          </button>
                        </a>
                        <a
                          href="https://snapshot.org/#/bitdao.eth/"
                          target="_blank"
                          rel="noreferrer noopener"
                          className="style-214"
                        >
                          <button type="button" className="style-215">
                            Snapshot
                          </button>
                        </a>
                        <a
                          href=" https://delegatevote.mantle.xyz/ "
                          target="_blank"
                          rel="noreferrer noopener"
                          className="style-216"
                        >
                          <button type="button" className="style-217">
                            Delegate
                          </button>
                        </a>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="style-218">
                  <div width={500} height={500} className="style-219">
                    <img
                      alt="Resource Management"

                      loading="eager"
                      decoding="async"
                      data-nimg="fill"
                      className="style-220"
                      sizes="100vw"
                      // srcSet="/_next/image?url=%2Frestaking-ilustration.png&w=640&q=75 640w, /_next/image?url=%2Frestaking-ilustration.png&w=750&q=75 750w, /_next/image?url=%2Frestaking-ilustration.png&w=828&q=75 828w, /_next/image?url=%2Frestaking-ilustration.png&w=1080&q=75 1080w, /_next/image?url=%2Frestaking-ilustration.png&w=1200&q=75 1200w, /_next/image?url=%2Frestaking-ilustration.png&w=1920&q=75 1920w, /_next/image?url=%2Frestaking-ilustration.png&w=2048&q=75 2048w, /_next/image?url=%2Frestaking-ilustration.png&w=3840&q=75 3840w"
                      src="/restaking-ilustration.png"
                    />
                  </div>
                  <div className="style-221">
                    <div className="style-222" />
                    <div className="style-223">
                      <h3 className="style-224">Resource Management</h3>
                      <p className="style-225">
                        As stewards of a significant treasury, and with the power to
                        shape economic outcomes of Mantle products, we prioritize a
                        value-oriented mindset, transparency, and accountability.
                      </p>
                      <div className="style-226">
                        <a
                          href="https://treasurymonitor.mantle.xyz/"
                          target="_blank"
                          rel="noreferrer noopener"
                          className="style-227"
                        >
                          <button type="button" className="style-228">
                            Treasury Monitor
                          </button>
                        </a>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="style-229">
              <h3 className="style-230">Treasury</h3>
              <div className="style-231">
                <div className="style-232">
                  <div className="style-233">
                    Symbol, <span className="style-234">Name</span>
                  </div>
                  <div className="style-235">
                    Tokens, <span className="style-236">Percentage</span>
                  </div>
                  <div className="style-237">USD</div>
                </div>
                <div className="style-238">
                  <div className="style-239">
                    <div data-before="Symbol, Name" className="style-240">
                      <div className="style-241">
                        <img
                          alt="Mantle"
                          loading="lazy"
                          width={24}
                          height={24}
                          decoding="async"
                          data-nimg={1}
                          // srcSet="/_next/image?url=https%3A%2F%2Fstatic.debank.com%2Fimage%2Feth_token%2Flogo_url%2F0x3c3a81e81dc49a522a592e7622a7e711c06bf354%2Fa443c78c33704d48f06e5686bb87f85e.png&w=32&q=75 1x, /_next/image?url=https%3A%2F%2Fstatic.debank.com%2Fimage%2Feth_token%2Flogo_url%2F0x3c3a81e81dc49a522a592e7622a7e711c06bf354%2Fa443c78c33704d48f06e5686bb87f85e.png&w=48&q=75 2x"
                          src="/a443c78c33704d48f06e5686bb87f85e.png"
                          className="style-242"
                        />
                      </div>
                      <div className="style-243">MNT</div>
                      <div className="style-244">Mantle</div>
                    </div>
                    <div data-before="Tokens, Percentage" className="style-245">
                      <div className="style-246">
                        2,977,553,903 <span className="style-247">MNT</span>
                      </div>
                      <div className="style-248">78.56%</div>
                    </div>
                    <div data-before="USD" className="style-249">
                      <div className="style-250">
                        <span className="style-251">$</span> 4,009,421,820
                      </div>
                    </div>
                  </div>
                  <div className="style-252">
                    <div data-before="Symbol, Name" className="style-253">
                      <div className="style-254">
                        <img
                          alt="mETH"
                          loading="lazy"
                          width={24}
                          height={24}
                          decoding="async"
                          data-nimg={1}
                          // srcSet="/_next/image?url=https%3A%2F%2Fstatic.debank.com%2Fimage%2Feth_token%2Flogo_url%2F0xd5f7838f5c461feff7fe49ea5ebaf7728bb0adfa%2Fc6a5196a03ec0dcb0f0b522b30579f61.png&w=32&q=75 1x, /_next/image?url=https%3A%2F%2Fstatic.debank.com%2Fimage%2Feth_token%2Flogo_url%2F0xd5f7838f5c461feff7fe49ea5ebaf7728bb0adfa%2Fc6a5196a03ec0dcb0f0b522b30579f61.png&w=48&q=75 2x"
                          src="/c6a5196a03ec0dcb0f0b522b30579f61.png"
                          className="style-255"
                        />
                      </div>
                      <div className="style-256">mETH</div>
                      <div className="style-257">mETH</div>
                    </div>
                    <div data-before="Tokens, Percentage" className="style-258">
                      <div className="style-259">
                        122,955 <span className="style-260">mETH</span>
                      </div>
                      <div className="style-261">8.23%</div>
                    </div>
                    <div data-before="USD" className="style-262">
                      <div className="style-263">
                        <span className="style-264">$</span> 419,894,527
                      </div>
                    </div>
                  </div>
                  <div className="style-265">
                    <div data-before="Symbol, Name" className="style-266">
                      <div className="style-267">
                        <img
                          alt="EigenLayer"
                          loading="lazy"
                          width={24}
                          height={24}
                          decoding="async"
                          data-nimg={1}
                          // srcSet="/_next/image?url=https%3A%2F%2Fstatic.debank.com%2Fimage%2Fproject%2Flogo_url%2Feigenlayer%2F60961d60d58c7619cf845ff06b2236af.png&w=32&q=75 1x, /_next/image?url=https%3A%2F%2Fstatic.debank.com%2Fimage%2Fproject%2Flogo_url%2Feigenlayer%2F60961d60d58c7619cf845ff06b2236af.png&w=48&q=75 2x"
                          src="/60961d60d58c7619cf845ff06b2236af.png"
                          className="style-268"
                        />
                      </div>
                      <div className="style-269">mETH</div>
                      <div className="style-270">EigenLayer</div>
                    </div>
                    <div data-before="Tokens, Percentage" className="style-271">
                      <div className="style-272">
                        100,006 <span className="style-273">mETH</span>
                      </div>
                      <div className="style-274">6.69%</div>
                    </div>
                    <div data-before="USD" className="style-275">
                      <div className="style-276">
                        <span className="style-277">$</span> 341,523,323
                      </div>
                    </div>
                  </div>
                  <div className="style-278">
                    <div data-before="Symbol, Name" className="style-279">
                      <div className="style-280">
                        <img
                          alt="Ethereum"
                          loading="lazy"
                          width={24}
                          height={24}
                          decoding="async"
                          data-nimg={1}
                          // srcSet="/_next/image?url=https%3A%2F%2Fstatic.debank.com%2Fimage%2Ftoken%2Flogo_url%2Feth%2F935ae4e4d1d12d59a99717a24f2540b5.png&w=32&q=75 1x, /_next/image?url=https%3A%2F%2Fstatic.debank.com%2Fimage%2Ftoken%2Flogo_url%2Feth%2F935ae4e4d1d12d59a99717a24f2540b5.png&w=48&q=75 2x"
                          src="/935ae4e4d1d12d59a99717a24f2540b5.png"
                          className="style-281"
                        />
                      </div>
                      <div className="style-282">ETH</div>
                      <div className="style-283">Ethereum</div>
                    </div>
                    <div data-before="Tokens, Percentage" className="style-284">
                      <div className="style-285">
                        43,625 <span className="style-286">ETH</span>
                      </div>
                      <div className="style-287">2.84%</div>
                    </div>
                    <div data-before="USD" className="style-288">
                      <div className="style-289">
                        <span className="style-290">$</span> 144,878,891
                      </div>
                    </div>
                  </div>
                  <div className="style-291">
                    <div data-before="Symbol, Name" className="style-292">
                      <div className="style-293">
                        <img
                          alt="USDe"
                          loading="lazy"
                          width={24}
                          height={24}
                          decoding="async"
                          data-nimg={1}
                          // srcSet="/_next/image?url=https%3A%2F%2Fstatic.debank.com%2Fimage%2Feth_token%2Flogo_url%2F0x4c9edd5852cd905f086c759e8383e09bff1e68b3%2F9ca0ec537ffcecb6026b47cce6437659.png&w=32&q=75 1x, /_next/image?url=https%3A%2F%2Fstatic.debank.com%2Fimage%2Feth_token%2Flogo_url%2F0x4c9edd5852cd905f086c759e8383e09bff1e68b3%2F9ca0ec537ffcecb6026b47cce6437659.png&w=48&q=75 2x"
                          src="/9ca0ec537ffcecb6026b47cce6437659.png"
                          className="style-294"
                        />
                      </div>
                      <div className="style-295">USDe</div>
                      <div className="style-296">USDe</div>
                    </div>
                    <div data-before="Tokens, Percentage" className="style-297">
                      <div className="style-298">
                        91,580,242 <span className="style-299">USDe</span>
                      </div>
                      <div className="style-300">1.80%</div>
                    </div>
                    <div data-before="USD" className="style-301">
                      <div className="style-302">
                        <span className="style-303">$</span> 91,683,095
                      </div>
                    </div>
                  </div>
                  <div className="style-304">
                    <div data-before="Symbol, Name" className="style-305">
                      <div className="style-306">
                        <img
                          alt="Ondo U.S. Dollar Yield"
                          loading="lazy"
                          width={24}
                          height={24}
                          decoding="async"
                          data-nimg={1}
                          // srcSet="/_next/image?url=https%3A%2F%2Fstatic.debank.com%2Fimage%2Fmnt_token%2Flogo_url%2F0x5be26527e817998a7206475496fde1e68957c5a6%2F4c1b30d2838237366a7bf62db849eed9.png&w=32&q=75 1x, /_next/image?url=https%3A%2F%2Fstatic.debank.com%2Fimage%2Fmnt_token%2Flogo_url%2F0x5be26527e817998a7206475496fde1e68957c5a6%2F4c1b30d2838237366a7bf62db849eed9.png&w=48&q=75 2x"
                          src="/4c1b30d2838237366a7bf62db849eed9.png"
                          className="style-307"
                        />
                      </div>
                      <div className="style-308">USDY</div>
                      <div className="style-309">Ondo U.S. Dollar Yield</div>
                    </div>
                    <div data-before="Tokens, Percentage" className="style-310">
                      <div className="style-311">
                        41,968,945 <span className="style-312">USDY</span>
                      </div>
                      <div className="style-313">0.85%</div>
                    </div>
                    <div data-before="USD" className="style-314">
                      <div className="style-315">
                        <span className="style-316">$</span> 43,408,903
                      </div>
                    </div>
                  </div>
                  <div className="style-317">
                    <div data-before="Symbol, Name" className="style-318">
                      <div className="style-319">
                        <img
                          alt="Tether USD"
                          loading="lazy"
                          width={24}
                          height={24}
                          decoding="async"
                          data-nimg={1}
                          // srcSet="/_next/image?url=https%3A%2F%2Fstatic.debank.com%2Fimage%2Fcoin%2Flogo_url%2Fusdt%2F23af7472292cb41dc39b3f1146ead0fe.png&w=32&q=75 1x, /_next/image?url=https%3A%2F%2Fstatic.debank.com%2Fimage%2Fcoin%2Flogo_url%2Fusdt%2F23af7472292cb41dc39b3f1146ead0fe.png&w=48&q=75 2x"
                          src="/23af7472292cb41dc39b3f1146ead0fe.png"
                          className="style-320"
                        />
                      </div>
                      <div className="style-321">USDT</div>
                      <div className="style-322">Tether USD</div>
                    </div>
                    <div data-before="Tokens, Percentage" className="style-323">
                      <div className="style-324">
                        31,276,643 <span className="style-325">USDT</span>
                      </div>
                      <div className="style-326">0.61%</div>
                    </div>
                    <div data-before="USD" className="style-327">
                      <div className="style-328">
                        <span className="style-329">$</span> 31,288,997
                      </div>
                    </div>
                  </div>
                  <div className="style-330">
                    <div data-before="Symbol, Name" className="style-331">
                      <div className="style-332">
                        <img
                          alt="USDC"
                          loading="lazy"
                          width={24}
                          height={24}
                          decoding="async"
                          data-nimg={1}
                          // srcSet="/_next/image?url=https%3A%2F%2Fstatic.debank.com%2Fimage%2Fcoin%2Flogo_url%2Fusdc%2Fe87790bfe0b3f2ea855dc29069b38818.png&w=32&q=75 1x, /_next/image?url=https%3A%2F%2Fstatic.debank.com%2Fimage%2Fcoin%2Flogo_url%2Fusdc%2Fe87790bfe0b3f2ea855dc29069b38818.png&w=48&q=75 2x"
                          src="/e87790bfe0b3f2ea855dc29069b38818.png"
                          className="style-333"
                        />
                      </div>
                      <div className="style-334">USDC</div>
                      <div className="style-335">USDC</div>
                    </div>
                    <div data-before="Tokens, Percentage" className="style-336">
                      <div className="style-337">
                        21,442,341 <span className="style-338">USDC</span>
                      </div>
                      <div className="style-339">0.42%</div>
                    </div>
                    <div data-before="USD" className="style-340">
                      <div className="style-341">
                        <span className="style-342">$</span> 21,442,341
                      </div>
                    </div>
                  </div>
                </div>
                <div className="style-343">
                  <span className="style-344">$</span>5,103,541,897
                </div>
              </div>
              <div className="style-345">
                <a
                  href="https://docs.mantle.xyz/governance/parameters/treasury#treasury-holdings-core"
                  target="_blank"
                  rel="noreferrer noopener"
                  className="style-346"
                >
                  <button type="button" className="style-347">
                    View wallets
                  </button>
                </a>
              </div>
            </div>
            <div className="style-348">
              <h2 className="style-349">
                We work with initiatives that drive ecosystem adoption
              </h2>
              <div className="style-350">
                <div className="style-351">
                  <div className="style-352">
                    <div className="style-353">
                      <div className="style-354">
                        <div className="style-355">
                          <img
                            alt="mantle logo"
                            loading="lazy"
                            width={56}
                            height={56}
                            decoding="async"
                            data-nimg={1}
                            className="style-356"
                            // srcSet="/_next/image?url=%2Flogo-mantle-minimal.png&w=64&q=75 1x, /_next/image?url=%2Flogo-mantle-minimal.png&w=128&q=75 2x"
                            src="/logo-mantle-minimal.png"
                          />
                        </div>
                        <h3 className="style-357">Mantle EcoFund</h3>
                      </div>
                      <p className="style-358">
                        A capital pool of $200M investing in high-potential applications
                        and technology partners of Mantle Products.
                      </p>
                      <div className="style-359">
                        <a href="/ecofund" className="style-360">
                          <button type="button" className="style-361">
                            Live
                          </button>
                        </a>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="style-362">
                  <div className="style-363">
                    <div className="style-364">
                      <div className="style-365">
                        <div className="style-366">
                          <img
                            alt="BitDAO logo"
                            loading="lazy"
                            width={56}
                            height={56}
                            decoding="async"
                            data-nimg={1}
                            className="style-367"
                            // srcSet="/_next/image?url=%2Flogo-edudao.png&w=64&q=75 1x, /_next/image?url=%2Flogo-edudao.png&w=128&q=75 2x"
                            src="/logo-edudao.png"
                          />
                        </div>
                        <h3 className="style-368">EduDAO</h3>
                      </div>
                      <p className="style-369">
                        An incubator for the innovators of tomorrow that empower
                        university groups and centers through resources and active
                        capital, aiming to foster blockchain innovation for the benefit
                        of all.
                      </p>
                      <div className="style-370">
                        <a
                          href="https://www.edudao.io/"
                          target="_blank"
                          rel="noreferrer noopener"
                          className="style-371"
                        >
                          <button type="button" className="style-372">
                            Live
                          </button>
                        </a>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="style-373">
                  <div className="style-374">
                    <div className="style-375">
                      <div className="style-376">
                        <div className="style-377">
                          <img
                            alt="BitDAO logo"
                            loading="lazy"
                            width={56}
                            height={56}
                            decoding="async"
                            data-nimg={1}
                            className="style-378"
                            // srcSet="/_next/image?url=%2Flogo-game7.png&w=64&q=75 1x, /_next/image?url=%2Flogo-game7.png&w=128&q=75 2x"
                            src="/logo-game7.png"
                          />
                        </div>
                        <h3 className="style-379">Game7</h3>
                      </div>
                      <p className="style-380">
                        A collective of gaming veterans and crypto-native leaders
                        passionate about building a more equitable gaming industry — one
                        where developers and players are on the same side.
                      </p>
                      <div className="style-381">
                        <a
                          href="https://game7.io/"
                          target="_blank"
                          rel="noreferrer noopener"
                          className="style-382"
                        >
                          <button type="button" className="style-383">
                            Live
                          </button>
                        </a>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="style-384">
              <h2 className="style-385">Mantle Ecosystem</h2>
              <div className="style-386">
                <picture className="style-387">
                  <source
                    srcSet="ecosystem-graphic-s.png"
                    media="(max-width: 500px)"
                    className="style-388"
                  />
                  <source
                    srcSet="ecosystem-graphic-m.png"
                    media="(max-width: 799px)"
                    className="style-389"
                  />
                  <source
                    srcSet="/ecosystem-graphic-l.png"
                    media="(min-width: 800px)"
                    className="style-390"
                  />
                  <img
                    alt="My image"
                    loading="lazy"
                    decoding="async"
                    data-nimg="fill"
                    sizes="100vw"
                    // srcSet="/_next/image?url=%2Fecosystem-graphic-l.png&w=640&q=100 640w, /_next/image?url=%2Fecosystem-graphic-l.png&w=750&q=100 750w, /_next/image?url=%2Fecosystem-graphic-l.png&w=828&q=100 828w, /_next/image?url=%2Fecosystem-graphic-l.png&w=1080&q=100 1080w, /_next/image?url=%2Fecosystem-graphic-l.png&w=1200&q=100 1200w, /_next/image?url=%2Fecosystem-graphic-l.png&w=1920&q=100 1920w, /_next/image?url=%2Fecosystem-graphic-l.png&w=2048&q=100 2048w, /_next/image?url=%2Fecosystem-graphic-l.png&w=3840&q=100 3840w"
                    src="/ecosystem-graphic-l.png"
                    className="style-391"
                  />
                </picture>
              </div>
            </div>
            <div className="style-392">
              <h2 className="style-393">Join the Community</h2>
              <div className="style-394">
                <div className="style-395">
                  <div className="style-396">
                    <div className="style-397">
                      <a
                        href="https://twitter.com/0xMantle"
                        target="_blank"
                        rel="noreferrer noopener"
                        className="style-398"
                      >
                        <button type="button" className="style-399">
                          Twitter
                        </button>
                      </a>
                    </div>
                  </div>
                  <div className="style-400">
                    <a
                      href="https://twitter.com/0xMantle"
                      target="_blank"
                      rel="noreferrer noopener"
                      className="style-401"
                    >
                      <img
                        alt="Twitter Logo"
                        loading="lazy"
                        width={54}
                        height={54}
                        decoding="async"
                        data-nimg={1}
                        className="style-402"
                        src="/X.10d54851.svg"
                      />
                    </a>
                  </div>
                </div>
                <div className="style-403">
                  <div className="style-404">
                    <div className="style-405">
                      <a
                        href="https://t.me/mantlenetwork"
                        target="_blank"
                        rel="noreferrer noopener"
                        className="style-406"
                      >
                        <button type="button" className="style-407">
                          Telegram
                        </button>
                      </a>
                    </div>
                  </div>
                  <div className="style-408">
                    <a
                      href="https://t.me/mantlenetwork"
                      target="_blank"
                      rel="noreferrer noopener"
                      className="style-409"
                    >
                      <img
                        alt="Telegram Logo"
                        loading="lazy"
                        width={54}
                        height={54}
                        decoding="async"
                        data-nimg={1}
                        className="style-410"
                        src="/telegram-logo.svg"
                      />
                    </a>
                  </div>
                </div>
                <div className="style-411">
                  <div className="style-412">
                    <div className="style-413">
                      <a
                        href="https://discord.com/invite/0xMantle"
                        target="_blank"
                        rel="noreferrer noopener"
                        className="style-414"
                      >
                        <button type="button" className="style-415">
                          Discord
                        </button>
                      </a>
                    </div>
                  </div>
                  <div className="style-416">
                    <a
                      href="https://discord.com/invite/0xMantle"
                      target="_blank"
                      rel="noreferrer noopener"
                      className="style-417"
                    >
                      <img
                        alt="Discord Logo"
                        loading="lazy"
                        width={54}
                        height={54}
                        decoding="async"
                        data-nimg={1}
                        className="style-418"
                        src="/discord-logo.svg"
                      />
                    </a>
                  </div>
                </div>
                <div className="style-419">
                  <div className="style-420">
                    <div className="style-421">
                      <a
                        href="/jobs"
                        target="_self"
                        rel="noreferrer noopener"
                        className="style-422"
                      >
                        <button type="button" className="style-423">
                          Careers
                        </button>
                      </a>
                    </div>
                  </div>
                  <div className="style-424">
                    <a
                      href="/jobs"
                      target="_self"
                      rel="noreferrer noopener"
                      className="style-425"
                    >
                      <img
                        alt="Careers Logo"
                        loading="lazy"
                        width={54}
                        height={54}
                        decoding="async"
                        data-nimg={1}
                        className="style-426"
                        src="/logo-light.svg"
                      />
                    </a>
                  </div>
                </div>
              </div>
            </div>
            <footer aria-labelledby="footerHeading" className="style-427">
              <div className="style-428">
                <div className="style-429">
                  <div className="style-430">
                    <div className="style-431">
                      <a href="/" className="style-432">
                        <img
                          alt="Mantle logo"
                          loading="lazy"
                          width={98}
                          height={24}
                          decoding="async"
                          data-nimg={1}
                          className="style-433"
                          src="/LOGO.919a8f2b.svg"
                        />
                      </a>
                    </div>
                    <div className="style-434">
                      <a
                        target="_blank"
                        className="style-435"
                        href="https://twitter.com/0xMantle"
                      >
                        <img
                          alt="nav-twitter"
                          loading="lazy"
                          width={10}
                          height={9}
                          decoding="async"
                          data-nimg={1}
                          src="/X.10d54851.svg"
                          className="style-436"
                        />
                      </a>
                      <a
                        target="_blank"
                        className="style-437"
                        href="https://discord.com/invite/0xMantle"
                      >
                        <img
                          alt="nav-discord"
                          loading="lazy"
                          width={12}
                          height={10}
                          decoding="async"
                          data-nimg={1}
                          src="/dicord.121e4489.svg"
                          className="style-438"
                        />
                      </a>
                      <a
                        target="_blank"
                        className="style-439"
                        href="https://t.me/mantlenetwork"
                      >
                        <img
                          alt="nav-telegram"
                          loading="lazy"
                          width={12}
                          height={12}
                          decoding="async"
                          data-nimg={1}
                          src="/telegram.19a5a5db.svg"
                          className="style-440"
                        />
                      </a>
                    </div>
                    <p className="style-441">© Mantle 2024.All rights reserved.</p>
                  </div>
                  <div className="style-442">
                    <div className="style-443">
                      <h5 className="style-444">Mantle Network</h5>
                      <ul className="style-445">
                        <li className="style-446">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-447"
                            href="https://www.mantle.xyz/ecosystem"
                          >
                            ecosystem
                          </a>
                        </li>
                        <li className="style-448">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-449"
                            href="/bridge"
                          >
                            bridge
                          </a>
                        </li>
                        <li className="style-450">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-451"
                            href="https://docs.mantle.xyz/governance/introduction/overview"
                          >
                            governance docs
                          </a>
                        </li>
                        <li className="style-452">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-453"
                            href="/grants"
                          >
                            grants
                          </a>
                        </li>
                        <li className="style-454">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-455"
                            href="/ecofund"
                          >
                            EcoFund
                          </a>
                        </li>
                      </ul>
                    </div>
                  </div>
                  <div className="style-456">
                    <div className="style-457">
                      <h5 className="style-458">Build</h5>
                      <ul className="style-459">
                        <li className="style-460">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-461"
                            href="https://docs-v2.mantle.xyz/"
                          >
                            docs
                          </a>
                        </li>
                        <li className="style-462">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-463"
                            href="https://explorer.mantle.xyz/"
                          >
                            block explorer
                          </a>
                        </li>
                        <li className="style-464">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-465"
                            href="https://0xmantle.instatus.com/"
                          >
                            status
                          </a>
                        </li>
                        <li className="style-466">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-467"
                            href="https://immunefi.com/bounty/mantlelsp/"
                          >
                            bug bounty
                          </a>
                        </li>
                        <li className="style-468">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-469"
                            href="https://github.com/mantlenetworkio"
                          >
                            github
                          </a>
                        </li>
                        <li className="style-470">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-471"
                            href="https://subgraph.mantle.xyz/"
                          >
                            mantle subgraph
                          </a>
                        </li>
                      </ul>
                    </div>
                  </div>
                  <div className="style-472">
                    <div className="style-473">
                      <h5 className="style-474">mETH</h5>
                      <ul className="style-475">
                        <li className="style-476">
                          <a
                            rel="noreferrer noopener"
                            target="_self"
                            className="style-477"
                            href="/meth"
                          >
                            about
                          </a>
                        </li>
                        <li className="style-478">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-479"
                            href="https://meth.mantle.xyz/stake"
                          >
                            stake
                          </a>
                        </li>
                        <li className="style-480">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-481"
                            href="https://meth.mantle.xyz/explore"
                          >
                            explore
                          </a>
                        </li>
                        <li className="style-482">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-483"
                            href="https://meth.mantle.xyz/stats"
                          >
                            stats
                          </a>
                        </li>
                        <li className="style-484">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-485"
                            href="https://docs.mantle.xyz/meth/introduction/overview"
                          >
                            docs
                          </a>
                        </li>
                      </ul>
                    </div>
                  </div>
                  <div className="style-486">
                    <div className="style-487">
                      <h5 className="style-488">Resources</h5>
                      <ul className="style-489">
                        <li className="style-490">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-491"
                            href="/blog/all?page=1"
                          >
                            blog
                          </a>
                        </li>
                        <li className="style-492">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-493"
                            href="/events"
                          >
                            events
                          </a>
                        </li>
                        <li className="style-494">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-495"
                            href="/jobs"
                          >
                            careers
                          </a>
                        </li>
                        <li className="style-496">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-497"
                            href="https://drive.google.com/drive/folders/1GGyOmob0i86mEUqBTPvZLAhgnzt3Oxaa"
                          >
                            brand assets
                          </a>
                        </li>
                        <li className="style-498">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-499"
                            href="/privacy-policy"
                          >
                            privacy policy
                          </a>
                        </li>
                        <li className="style-500">
                          <a
                            rel="noreferrer noopener"
                            target="_blank"
                            className="style-501"
                            href="/terms"
                          >
                            terms
                          </a>
                        </li>
                      </ul>
                    </div>
                  </div>
                  <div className="style-502">
                    <img
                      alt="language select"
                      loading="lazy"
                      width={24}
                      height={24}
                      decoding="async"
                      data-nimg={1}
                      className="style-503"
                      src="/LangLogo.e26e1c25.svg"
                    />
                    <div className="style-504">
                      <div
                        role="menubar"
                        className="style-505"
                        tabIndex={0}
                        data-orientation="horizontal"
                      >
                        <button
                          type="button"
                          role="menuitem"
                          aria-haspopup="menu"
                          aria-expanded="false"
                          data-state="closed"
                          className="style-506"
                          tabIndex={-1}
                          data-orientation="horizontal"
                          data-radix-collection-item=""
                        >
                          <span className="style-507">en</span>
                          <svg
                            viewBox="0 0 12 13"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                            className="style-508"
                          >
                            <path
                              d="m3 5 3 3 3-3"
                              stroke="white"
                              strokeWidth="1.2"
                              className="style-509"
                            />
                          </svg>
                        </button>
                      </div>
                    </div>
                    <div className="style-510">
                      <div
                        role="menubar"
                        className="style-511"
                        tabIndex={0}
                        data-orientation="horizontal"
                      >
                        <button
                          type="button"
                          role="menuitem"
                          aria-haspopup="menu"
                          aria-expanded="false"
                          data-state="closed"
                          className="style-512"
                          tabIndex={-1}
                          data-orientation="horizontal"
                          data-radix-collection-item=""
                        >
                          <span className="style-513">en</span>
                          <svg
                            viewBox="0 0 12 13"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                            className="style-514"
                          >
                            <path
                              d="m3 5 3 3 3-3"
                              stroke="white"
                              strokeWidth="1.2"
                              className="style-515"
                            />
                          </svg>
                        </button>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </footer>
          </div>
          <div className="style-516" />
          <div className="style-517" />
        </div>
      </div>

    </>
  )
}

export default App
